...
<title><fmt:message key="reportDirection.title"/></title>
</head>
<body>
<div class="container">
...
<form name="locales" action="/delivery/Controller" method="post">
	...
	<input type="hidden" name="goTo" value="manager/report_direction.jsp">
</form>
...
<h1><fmt:message key="reportDirection.header"/></h1>
<form name="reportDirectionForm" action="/delivery/Controller" method="post">
<table class="table">
<thead>
<tr>
<th><fmt:message key="reportDirection.Direction"/></th>
<th><fmt:message key="reportDirection.Turnover"/></th>
</tr>
</thead>	
<c:if test="${reportDirectionList.size() > 0}">
<c:forEach var="s" items="${reportDirectionList}">
	<tr>
		<td>${s.getIndex()}</td>
		<td>${s.getValue()}</td>
	</tr>
...
<c:if test="${reportDirectionList.size() == 0}">
	<fmt:message key="reportDirection.noData"/>
</c:if>
</table>
</form>
